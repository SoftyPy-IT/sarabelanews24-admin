...
import { useGetSingleVideoNewsQuery } from "@/redux/dailynews/videoNews.api ";
...
const Update = ({ params }: newsProps) => {
  ...
  // const { data: singleData } = useGetSingleAdvertisementQuery(id);
  const { data: singleData, error } = useGetSingleAdvertisementQuery(id ?? "");
  ...
  const onSubmit = async (data: Inputs) => {
    const transformedData = {
      ...
      // advertisementImage: mainSelectedFiles.map((item) => item.url),
    ...
    try {
      const res = await updateAdvertisement(transformedData).unwrap();
  ...
  <div key={index} className="rounded-lg">
    <Image
      ...
      height={150}
      className="h-[150px] w-[150px] rounded-lg"
      unoptimized // <-- Add this to disable Next.js optimization
    />
  </div>
))}
