...
const AddImageForm = () => {
  const [mainSelectedFiles, setMainSelectedFiles] = React.useState<{ url: string }[]>([]);
  const [tagSelectedFiles, setTagSelectedFiles] = React.useState<{ url: string }[][]>([]);
  const [openSheetIndex, setOpenSheetIndex] = useState<number | null>(null);
  const [createPhotoNews] = useCreatePhotoNewsMutation({});
  const router = useRouter();
  ...
  //     tagImages: tagSelectedFiles.flat().map((item) => item.url).join(","),
  //     tags: data.galleries.map((tag, index) => ({
  //       imgTagline: tag.imageTagline,
  //       image: tagSelectedFiles[index]?.map((item) => item.url).join(",") || "",
  //     })),
  //   };
  //   console.log("modify value:", modifyData);
            ...
            <div className="grid grid-cols-12 gap-4 xl:6">
                  ...
                  <div>
                    <Sheet>
                      ...
                      <SheetContent side="right" style={{ maxWidth: "800px" }} className="pt-4 overflow-y-auto">
                        ...
                        <AllImgModal onImageSelect={handleImageSelect} onClose={() => setOpenSheetIndex(null)} />
                      </SheetContent>
                    </Sheet>
                  </div>

                  {mainSelectedFiles.map((file, index) => (
                    <Image key={index} src={file.url} alt={`Preview ${index}`} width={130} height={100} />
                  ))}
                  <div className="space-y-2">
                    <div className="col-span-2">
                      <TextInput control={form.control} name="title" placeholder="শিরোনাম" rules={{ required: "News title is required" }} />
                    </div>
              ...
              <div className="lg:col-span-4 col-span-full space-y-5">
                ...
                <section className="bg-white border border-gray-300 rounded p-5">
                  ...
                  <div className="col-span-2">
                    {fields.map((field, index) => (
                      <div key={field.id} className="flex flex-col space-y-3">
                        <div className="flex justify-between items-center gap-2 p-4">
                          <Sheet open={openSheetIndex === index} onOpenChange={(open) => setOpenSheetIndex(open ? index : null)}>
                            <SheetTrigger asChild>
                              <Button variant="outline" className="p-8 border rounded-full mb-2">
                                <ImageUpIcon color="red" size={50} /> Add Image
                              </Button>
                            </SheetTrigger>
                            <SheetContent side="right" className="pt-4 overflow-y-auto" style={{ maxWidth: "800px" }}>
                              <SheetTitle>সংবাদ ট্যাগ</SheetTitle>
                              <AllImgModal
                                onImageSelect={(images: any) => {
                                  const newTagFiles = [...tagSelectedFiles];
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            {index === fields.length - 1 && (
                              <Button type="button" variant="default" size="sm" onClick={appendField}>
                                <PlusIcon className="w-4 h-4" />
                              </Button>
                            )}
                          </div>
                        ...
                        {tagSelectedFiles[index]?.map((file, imgIndex) => (
                          <Image key={imgIndex} src={file.url} alt={`Preview ${imgIndex}`} width={130} height={100} />
                        ))}
                ...
                <section className="bg-white border border-gray-300 rounded p-5">
                  ...
                  <div className="col-span-2">
                    <div className="col-span-2">
                      <TextInput control={form.control} name="adminName" placeholder="Admin Name" rules={{ required: "Admin name is required" }} />
                    </div>
                    <div className="grid grid-cols-1 gap-4 mt-2">
