...
const NewsDataTable = () => {
  ...
  const newsData = fetchAllNews?.map((item: any, index: any) => ({
    id: item._id,
    slNo: index + 1,
  ...
  const columns: ColumnDef<any, any>[] = [
      ...
      cell: ({ row }) => {
        ...
        return imageUrl && imageUrl !== "N/A" ? (
          <div className="relative w-24 h-16">
            <Image src={imageUrl || "/placeholder.svg"} alt="News thumbnail" fill className="object-cover rounded-md" />
          </div>
        ) : (
    ...
    {
      accessorKey: "newsTag",
      header: () => <div className="w-40  font-bold flex justify-center items-center ">Tags</div>,
      cell: ({ row }) => (
        <div className="grid grid-cols-2 gap-1 place-content-center place-items-center">
          {row.original.newsTag.map((tag: string, index: number) => (
            <Badge key={index} variant="outline" className={`px-3 py-1 rounded-full ${tagColors[index % tagColors.length]}`}>
              {tag}
            </Badge>
          ))}
        </div>
